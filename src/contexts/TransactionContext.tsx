...
import { formatMonthlyTransactions, MonthlyDataProps } from "@/features/dashboard/metrics/utils";
...
export function TransactionProvider({ children }: { children: ReactNode }) {
  ...
  const [transactionsBackup, setTransactionsBackup] = useState<TransactionsProps[]>([]);
  ...
  const stateTypes = Array.from(new Set(transactions.map((t) => t.state))).map(
    (state) => ({
      label: state,
      value: state,
    })
